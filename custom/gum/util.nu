export def gum-path []: nothing -> string {
    ...
    if ($result | is-empty) {
        error make --unspanned {msg: "gum is not installed or not in PATH. Install: https://github.com/charmbracelet/gum"}
    }
    $result.0.path
}
...
export def is-interrupted [exit_code: int]: nothing -> bool {
