...
export def build-create-output [target: path, copy_errors: string]: nothing -> string {
    let output = [
...
export def handle-worktree-removal [worktree_path: path, workdir: path, force: bool]: nothing -> string {
...
export def handle-branch-deletion-direct [workdir: path, force: bool, branch_name?: string]: nothing -> string {
    if $branch_name == null {
...
export def validate-git-repo [spanned_workdir: record<value: path, span: any>]: nothing -> nothing {
    let workdir = $spanned_workdir.value
    if (do -i {
        git -C $workdir rev-parse --is-inside-work-tree | complete
    } | get exit_code) != 0 {
...
export def validate-directory-exists [spanned_dir: record<value: path, span: any>, context: string = directory]: nothing -> nothing {
...
export def branch-exists [branch: string, workdir: path = .]: nothing -> bool {
    (do -i {
        git -C $workdir rev-parse --verify $branch | complete
    } | get exit_code) == 0
}
# Validate that a branch exists, error if it doesn't
export def validate-branch-exists [spanned_branch: record<value: string, span: any>, spanned_workdir: record<value: path, span: any>]: nothing -> nothing {
...
export def validate-branch-available [spanned_branch: record<value: string, span: any>, spanned_workdir: record<value: path, span: any>]: nothing -> nothing {
    let branch = $spanned_branch.value
    let workdir = $spanned_workdir.value
...
export def resolve-worktree-path [branch: string, workdir: path, custom_path?: path]: nothing -> string {
...
export def get-current-branch [workdir: path = .]: nothing -> string {
...
export def create-worktree [
    ...
    let result = (do -i {
        ^git -C $workdir worktree add -b $branch $target $base_branch | complete
    })
...
export def copy-to-clipboard [path: path]: nothing -> string {
    try {
...
export def get-worktree-list [workdir: path = .]: nothing -> table {
    ^git -C $workdir worktree list
    | lines
    | split column -r '\s+' path hash branch
}
# Get branch name from worktree path
export def get-worktree-branch [worktree_path: path, workdir: path = .]: nothing -> string {
    let worktree_info = (
...
export def get-remote-tracking-branch [branch: string, workdir: path = .]: nothing -> string {
    let remote = (do -i {
        git -C $workdir config --get $"branch.($branch).remote" | complete
    })
...
export def worktree-is-dirty [worktree_path: path]: nothing -> bool {
...
export def worktree-exists [worktree_path: path, workdir: path = .]: nothing -> bool {
    get-worktree-list $workdir
    | where path == $worktree_path
    | is-not-empty
}
# Remove a worktree using git
export def remove-worktree-git [worktree_path: path, workdir: path = ., --force]: nothing -> nothing {
    let remove_args = if $force { [--force] } else { [] }
    let result = (do -i {
        ^git -C $workdir worktree remove ...$remove_args $worktree_path | complete
    })
    if $result.exit_code != 0 {
        error make --unspanned {
            msg: $"Failed to remove worktree: ($result.stderr | str trim)"
        }
    }
}
# Remove an orphaned worktree directory (not tracked by git)
export def remove-orphaned-directory [worktree_path: path]: nothing -> nothing {
    try {
        rm -rf $worktree_path
    } catch {|err| error make --unspanned {
        msg: $"Failed to remove orphaned directory: ($err.msg)"
    } }
}
# Delete a local branch
export def delete-local-branch [branch: string, workdir: path = ., --force]: nothing -> record<success: bool, message: string> {
...
export def delete-remote-branch [remote: string, branch: string, workdir: path = .]: nothing -> record<success: bool, message: string> {
...
export def build-gum-options [has_remote: bool]: nothing -> list<string> {
    if $has_remote {
...
export def prompt-with-gum [header: string, options: list<string>]: nothing -> string {
    let choice = (do -i {
...
export def resolve-worktree-path-for-removal [path_input: path, current_dir: path]: nothing -> string {
...
export def get-worktree-path-by-branch [branch: string, workdir: path = .]: nothing -> string {
    let worktree_info = (
        get-worktree-list $workdir
        | where branch == $"[($branch)]"
        | get 0?
