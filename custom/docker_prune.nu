...
export def dockerprune [
  --force(-f)        # Skip confirmation prompt
    ...
    if not ($containers | is-empty) {
        ...
        } catch {|err| print -e $" - Failed to remove containers: ($err.msg)" }
    }
    # Images
    if not ($images | is-empty) {
        ...
        } catch {|err| print -e $" - Failed to remove images: ($err.msg)" }
    }
    # Volumes
    if not ($volumes | is-empty) {
        ...
        } catch {|err| print -e $" - Failed to remove volumes: ($err.msg)" }
    }
    # System prune
    try {
    ...
    } catch {|err| print -e $" - Failed to prune system: ($err.msg)" }
}
